...
import 'package:fanpage_app/new_message.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton:
         FloatingActionButton(
                  onPressed: () {
                    Navigator.of(context).push(MaterialPageRoute(builder: (context) => const NewMessageScreen()));
                  },
                  child: const Text("+"),
                ),
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          return ListView(
            ...
            return ListTile(title: Text(users["display_name"]));
          }).toList());
        },
      ),
    );
  ...
  void addPost() async {
    await _db.collection("posts").add({"message": _postText.text});
  }
